...
Before rebasing, record the feature HEAD and read [Plan mode acceptance contract](references/plan-mode-acceptance.md). Use that contract and the pre-rebase behavior as the test oracle. Review changed test assertions against the contract; never change an expected Plan-scoped event to a global update merely to make rebased tests pass.
...
Complete the acceptance contract after the final rebase, including its different-model case. Passing tests with weakened assertions is not verification. A missing or failed required scenario blocks publication; record the tested commit and results.
...
Do not push by default. Run `git push --force-with-lease` only when the user explicitly asks to update the remote feature branch. Invoking `$123-release-fork-codex` is explicit authorization to update that branch as part of preparing the release; it never authorizes a force-push of `main`.
