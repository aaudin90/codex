---
name: 123-release-fork-from-upstream
...
Orchestrate `$123-rebase-plan-mode`, `$123-release-fork-codex`, and `$123-clean-rust-artifacts` in that order. Accept an upstream version matching `X.Y.Z[-alpha…|-beta…]` and an optional fork version `X.Y.Z[-prerelease]`; a run is fresh-only and never resumes a partially published release. The fork version names the downstream `fork-v…` tag and Release without changing the upstream Cargo version. Run every Git action on `plan-mode-model-selection`; never check out, rebase, or push `main`.
...
```bash
python3 .codex/skills/123-release-fork-from-upstream/scripts/resolve_upstream_release.py <version>
```
...
Invoke `$123-rebase-plan-mode` on `plan-mode-model-selection` in two steps. First integrate the updated fork main, then move only the feature commits to the immutable release base:
...
Before publication, complete the [Plan mode acceptance contract](../123-rebase-plan-mode/references/plan-mode-acceptance.md) against the final candidate commit, including changed-test review and the candidate TUI smoke check. Carry its commit and results into the completion report. Do not treat a successful build or package verification as evidence that model selection preserves Plan mode; missing or failed acceptance checks block publication and subsequent cleanup.

Invoke `$123-release-fork-codex` for the upstream version and optional fork version, first with `--dry-run`, then `--publish`. For example, use upstream `0.147.0` with fork version `0.147.0-test` to create `fork-v0.147.0-test`. It must verify the exact upstream commit is an ancestor, build the canonical macOS ARM64 package, create the annotated fork tag at current `HEAD`, upload the package archive and SHA-256, and create the GitHub Release in `aaudin90/codex`.
...
Then invoke `$123-clean-rust-artifacts` from the repository root:

```bash
python3 .codex/skills/123-clean-rust-artifacts/scripts/clean_rust_artifacts.py --workspace codex-rs --scope all
python3 .codex/skills/123-clean-rust-artifacts/scripts/clean_rust_artifacts.py --workspace codex-rs --scope all --apply
```
