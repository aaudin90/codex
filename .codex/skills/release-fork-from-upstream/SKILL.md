...
Orchestrate `$rebase-plan-mode`, `$release-fork-codex`, and `$clean-rust-artifacts` in that order. Accept an upstream version matching `X.Y.Z[-alpha…|-beta…]`; a run is fresh-only and never resumes a partially published release. Run every Git action on `plan-mode-model-selection`; never check out, rebase, or push `main`.
...
2. Refuse to start if local or remote `fork-v<version>` already exists, or GitHub already has that release. Do not delete, retag, overwrite, or resume it.
...
Invoke `$release-fork-codex` for the same version, first with `--dry-run`, then `--publish`. It must verify the exact upstream commit is an ancestor, build macOS ARM64 `codex-cli`, create annotated `fork-v<version>` at current `HEAD`, upload the binary and SHA-256, and create the GitHub Release in `aaudin90/codex`.
