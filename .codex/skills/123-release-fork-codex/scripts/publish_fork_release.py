...
def ensure_release_assets(
    root: Path, fork_tag: str, artifact: Path, checksum: Path, *, is_draft: bool
) -> None:
    release = json.loads(
        output(
            [
    ...
    if (
        release["tagName"] != fork_tag
        or release["isDraft"] != is_draft
        or not all(
            assets.get(path.name, {}).get("size") == path.stat().st_size
            and assets[path.name].get("state") == "uploaded"
            for path in expected
        )
    ):
        raise CommandFailed(f"GitHub release {fork_tag} is missing verified assets")
...
def main() -> int:
    ...
    try:
        ...
        ensure_release_assets(root, fork_tag, artifact, checksum, is_draft=True)
        run(
            [
                "gh",
                "release",
        ...
        ensure_release_assets(root, fork_tag, artifact, checksum, is_draft=False)
    finally:
        artifacts.cleanup()
