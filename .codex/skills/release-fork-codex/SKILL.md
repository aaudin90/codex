...
Publish the current `plan-mode-model-selection` `HEAD` to `aaudin90/codex` as `fork-v<upstream-version>`. The release includes downstream changes, including Plan mode, and is not the upstream release commit. Never check out, rebase, or push `main`.

## Inputs and safety

Accept only an upstream version matching `X.Y.Z[-alpha…|-beta…]`; normalize it to `rust-v<version>`. Require `git branch --show-current` to print exactly `plan-mode-model-selection`. The local workspace version in `codex-rs/Cargo.toml` must already equal that version. Never create a version-bump commit.
