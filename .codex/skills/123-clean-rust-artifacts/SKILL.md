...
```bash
python3 .codex/skills/123-clean-rust-artifacts/scripts/clean_rust_artifacts.py --workspace codex-rs
```
...
```bash
python3 .codex/skills/123-clean-rust-artifacts/scripts/clean_rust_artifacts.py \
...
Do not remove `~/.rustup/toolchains` or `~/.cargo/bin` in normal cleanup: they are installed tools, not disposable build cache. If the user explicitly wants to uninstall every Rust toolchain, first report the exact size and explain that Cargo/Rust will need reinstalling. Only then use both `--include-toolchains` and `--confirm-toolchain-removal` with `--apply`. In this fork, `$123-release-fork-from-upstream` restores the pinned `codex-rs/rust-toolchain.toml` channel and components before a future release build; preserve `~/.cargo/bin` so `rustup` remains available.

For the verified fork-release cleanup invoked by `$123-release-fork-from-upstream`, remove installed toolchains as part of the requested full cleanup. Add `--include-toolchains --confirm-toolchain-removal` to both the report and apply commands after all Cargo, rustc, and sccache processes have exited. This intentionally trades disk space for a pinned-toolchain reinstall before the next release.
